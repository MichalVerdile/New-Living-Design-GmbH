...
function buildPrompt(v: {
  ...
  const toilet = v.cistern === 'aufputz'
    ...
    : `the cistern stays hidden exactly where it already is, and the wall keeps the exact shape it has in image 1: never invent a half-height wall, a low built wall, a boxed pre-wall, a ledge or a shelf that image 1 does not show, whether behind the toilet, the washbasin or the shower. If the toilet in image 1 hangs directly against a flat wall, tiled or plastered floor to ceiling, with no low wall, ledge or shelf anywhere near it, that same flat, full-height wall stays exactly as it is, only newly tiled, with nothing new added in front of it. If instead the toilet in image 1 already hangs on a visible half-height wall, a low built wall or boxed pre-wall that carries the cistern and often has a shelf on top, that low wall is part of the room and stays: same place, same length, same height, same depth, only newly tiled, and the toilet stays mounted on its front at exactly the same spot, at the same distance from the door and from the corner. Do not flatten it, do not extend it to the ceiling and do not push the toilet back against the wall behind it. In both cases no new free-standing module is added. The toilet is wall-hung, rimless, in ${v.sanitaryPrompt}, with seat and lid in the very same ${v.sanitaryPrompt}, never wood, never a contrasting colour, at exactly its existing position`;
