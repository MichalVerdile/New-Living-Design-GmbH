...
function buildPrompt(v: {
  ...
  return [
    ...
    `Changes (package "${v.packageName}"): the floor tiled with ${v.format} cm ${v.tilePrompt} tiles${asSample}; ${v.wallPrompt}; ${v.showerPrompt} where the bathtub/shower is now; if a toilet is visible in image 1, a wall-hung rimless toilet in ${v.sanitaryPrompt} at exactly its existing position; if a washbasin is visible in image 1, ${v.basinPrompt} on a wall-hung vanity in ${v.furniturePrompt} at its existing place with ${v.mirrorPrompt} above it; ${v.finishPrompt} fittings (${v.tapSeries}). Remove clutter, towels, bottles, shower curtain and rugs. Natural daylight, no people, no text.`,
  ].join('\n');
}
