...
async function checkOpenings(
  ...
  const question =
    ...
    'For that floor-level shower in image 2, set drain_on_long_side true only if its channel drain runs along a long side of the shower floor rather than along a short end, and set shower_fittings_split true only if its fittings (mixer, overhead shower arm, hand shower holder) are mounted on two or more different walls rather than all on one wall; false when there is no such shower or you cannot see it. ' +
